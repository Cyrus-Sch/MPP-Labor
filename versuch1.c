...
int main(){
    ...
    // PWM
    SYSCTL_RCGCTIMER_R= 0x00000009; w++; // clock enable timer3 and timer1
    ...
    //
     ...
     ADC0_CC_R = 0x01 ; w++; // select PIOSC (internal RCOsc) as ADC analog clock
    ...
    printf("0x%3x=%4d (dec) ==> %04d [mV] \n", ADCoutput, ADCoutput, (int) ( ADCoutput * V_COEFF + 0.5));
        ...
        inc_dec = -1 * (((ADCoutput * V_COEFF + 0.5) - 2000)/150); // should range -20 to 20
        if ((TIMER3_TBMATCHR_R + inc_dec) > STARTVALUE - 80){inc_dec= 0;} // maximal duty cycle reached so we count down
        if ((TIMER3_TBMATCHR_R + inc_dec) < 80) {inc_dec= 0;} // min duty cycle reached so we stop.
        TIMER3_TBMATCHR_R = TIMER3_TBMATCHR_R + inc_dec; // adjusting the duty cycle

        for(w = 0; w < 10; w++){
            if (GPIO_PORTM_DATA_R == 0x02 && (TIMER3_TBMATCHR_R >= (STARTVALUE - 80))){
                            TIMER3_TBMATCHR_R = 80;
                            clockWait(100);
                            while(GPIO_PORTM_DATA_R == 0x02);
                            break;
                            }
            else if(GPIO_PORTM_DATA_R == 0x02 && (TIMER3_TBMATCHR_R <= (STARTVALUE - 80))){
                            TIMER3_TBMATCHR_R = (STARTVALUE - 80);
                            while(GPIO_PORTM_DATA_R == 0x02);
                            break;
                            }
        }
